...
interface SuspiciousActivityReportDialogProps {
  children: React.ReactNode;
}

export function SuspiciousActivityReportDialog({
  children,
}: SuspiciousActivityReportDialogProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isSubmitted, setIsSubmitted] = useState(false);

  const form = useForm<SuspiciousActivityData>({
    resolver: zodResolver(suspiciousActivitySchema),
    defaultValues: {
      reporterName: "",
      reporterEmail: "",
      description: "",
  ...
  async function onSubmit(data: SuspiciousActivityData) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Report suspected scams, fraud, fake profiles, or other suspicious
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Suspicious Activity specific fields */}
            <div className="grid gap-4 md:grid-cols-2">
              <FormField
                control={form.control}
                name="activityType"
            ...
            <div className="rounded-md bg-muted/50 p-4">
              <p className="text-muted-foreground text-sm">
                ...
                our security team and used only to investigate and resolve the
